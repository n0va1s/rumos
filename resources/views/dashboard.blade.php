<x-app-layout>
    ...
    <div class="p-6">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-3 md:gap-2">
            ...
            @foreach($quantityByGender as $gender)
            <div class="flex items-center p-4 bg-white rounded">
                <div class="flex items-center justify-center flex-shrink-0 w-16 h-16 bg-green-200 rounded">
                    <svg class="w-6 h-6 text-green-700 fill-current" xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 20 20" fill="currentColor">
                ...
                <div class="flex flex-col flex-grow ml-4">
                    <span class="text-xl font-bold">{{ $gender->qtd }}</span>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-500">Sexo {{$gender->title}}</span>
                        <span class="ml-2 text-sm font-semibold text-green-500">+28.4% / 2022</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center p-4 bg-white rounded">
                ...
                <div class="flex flex-col flex-grow ml-4">
                    <span
                        class="text-xl font-bold">{{ $course->community->title }}-{{ $course->number }}/{{ date('Y', strtotime($course->starts_at)) }}</span>
                    <div class="flex items-center justify-between">
